...
import { Wallet, ShieldCheck, Tag, CheckCircle2, Pencil, X } from "lucide-react";
import { useMerchantWalletStore, VerificationStatus } from "@/stores/useMerchantWalletStore";
...
export default function WalletOverview() {
  ...
  const shortAddress = merchantWallet.walletAddress
    ? merchantWallet.walletAddress.slice(0, 6) + "..." + merchantWallet.walletAddress.slice(-4)
    : "";
          ...
          <div className="relative mt-6 flex flex-col gap-6 xl:flex-row xl:items-center">
            <div className="flex w-full items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-6 xl:w-auto xl:min-w-[320px]">
              ...
              <div className="min-w-0 flex-1">
                ...
                {hasWallet ? (
                  <>
                    <p className="mt-1 truncate font-medium text-white">{shortAddress}</p>
                  ...
                  <>
                    <p className="mt-1 truncate font-medium text-zinc-500">Not Connected</p>
                    <button
                      onClick={handleOpenModal}
            ...
            <div className="grid w-full flex-1 grid-cols-1 gap-4 md:grid-cols-3">
              <div className="min-w-0 border-l border-white/10 py-2 pl-5 lg:pl-8">
                <p className="whitespace-nowrap text-sm text-zinc-500">Verification Status</p>
                <div className="mt-3 flex items-center gap-3">
                  <ShieldCheck className={hasWallet ? "h-6 w-6 shrink-0 text-[#b8ff3c]" : "h-6 w-6 shrink-0 text-zinc-600"} />
                  <span className={hasWallet ? "text-lg font-medium text-white" : "text-lg font-medium text-zinc-600"}>
                    {hasWallet ? (merchantWallet.verificationStatus || "PENDING") : "Not Verified"}
                  </span>
                </div>
              </div>

              <div className="min-w-0 border-l border-white/10 py-2 pl-5 lg:pl-8">
                <p className="whitespace-nowrap text-sm text-zinc-500">Wallet Label</p>
                <div className="mt-3 flex items-center gap-3">
                  <Tag className={hasWallet ? "h-6 w-6 shrink-0 text-[#b8ff3c]" : "h-6 w-6 shrink-0 text-zinc-600"} />
                  <span className={hasWallet ? "text-lg font-medium text-white" : "text-lg font-medium text-zinc-600"}>
                    {hasWallet ? (merchantWallet.label || "No Label") : "No Label"}
                  </span>
                </div>
              </div>

              <div className="min-w-0 border-l border-white/10 py-2 pl-5 lg:pl-8">
                <p className="whitespace-nowrap text-sm text-zinc-500">Default Status</p>
                <div className="mt-3 flex items-center gap-3">
                  <CheckCircle2 className={hasWallet ? "h-6 w-6 shrink-0 text-[#b8ff3c]" : "h-6 w-6 shrink-0 text-zinc-600"} />
                  <span className={hasWallet ? "text-lg font-medium text-white" : "text-lg font-medium text-zinc-600"}>
                    {hasWallet ? (merchantWallet.isDefault ? "True" : "False") : "False"}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#05080A] p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-semibold text-white">Edit Wallet</h2>
              <button
              ...
              {!showDetails ? (
                ...
                <div className="space-y-5">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <p className="text-xs text-zinc-500">Wallet Address</p>
                    <p className="mt-2 break-all font-mono text-white">{address}</p>
                  </div>

                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <p className="text-xs text-zinc-500">Chain ID</p>
                    <p className="mt-2 text-lg font-medium text-white">{chainId}</p>
                  </div>
